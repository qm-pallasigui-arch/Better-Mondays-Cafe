...
public class MenuMaintenancePanel extends JPanel {
    ...
    private void reload() {
        try {
            ...
            for (MenuItem item : repo.findAll()) {
                model.addRow(new Object[]{item.getName(), item.getCategory(), item.getHotPrice(), item.getIcedRegularPrice(), item.getIcedLargePrice()});
            }
        } catch (Exception e) {
    ...
    private void onEdit() {
        ...
        try {
            ...
            String newName = JOptionPane.showInputDialog(this, "Name:", item.getName()); if (newName==null) return;
            String newCat = JOptionPane.showInputDialog(this, "Category:", item.getCategory()); if (newCat==null) return;
            double hot = Double.parseDouble(JOptionPane.showInputDialog(this, "Hot price:", item.getHotPrice()));
            double reg = Double.parseDouble(JOptionPane.showInputDialog(this, "Iced regular price:", item.getIcedRegularPrice()));
            double large = Double.parseDouble(JOptionPane.showInputDialog(this, "Iced large price:", item.getIcedLargePrice()));
            MenuItem edited = createMenuItem(newCat.trim(), newName.trim(), hot, reg, large);
            String ing = JOptionPane.showInputDialog(this, "Ingredients (name:qty,name:qty):", ingredientsToCsv(item.getIngredients()));
    ...
    private void onDelete() {
        ...
        try { repo.delete(name); reload(); } catch (Exception e) { JOptionPane.showMessageDialog(this, "Delete failed: " + e.getMessage()); }
    }
    ...
    private void parseAndSetIngredients(MenuItem item, String csv) {
        ...
        for (String p : parts) {
            String[] kv = p.split(":", 2);
            if (kv.length == 2) {
                try { item.addIngredient(kv[0].trim(), Double.parseDouble(kv[1].trim())); } catch (Exception ignored) {}
            }
        }
    }
